...
mission_util.doAgentBanter = function(script,sim,cross_script,odds,returnIfFailed)
	...
	if #agents	> 0 and sim:getParams().world ~= "omni" then
		...
		local cross_set = {} --this used to be a copy of cross_script, now empty -K
		local speechQue = {}
		...
		if sim:isVersion("0.17.5") then
			...
			while #cross_set > 0 and dialogueSet == nil do -- find a set not already used
				...
				for i,crossId in ipairs(agency.crossIds or {})do
					if dialogueSet and crossId == dialogueSet.id then -- if dialogueSet is not nil, and if crossId equals the id from dialogueSet (added below), remove this banter from banter pool!
						dialogueSet = nil
			...
			if dialogueSet and sim:nextRand() < (odds or sim:getParams().difficultyOptions.banter or 0.2) then
				...
				for i,event in ipairs(dialogueSet.dialogue) do				
					...
					table.insert(speechQue,speech) --makes lines for convo
					...
					-- end
					-- ^: old vanilla code, causes bugs on Rewind
					--second half of it was moved to doFinishMission in modinit
					
					if dialogueSet.id then					
					sim.usedbanterID = dialogueSet.id
					end
					-------------------------------------------------------------
				end			
			end
